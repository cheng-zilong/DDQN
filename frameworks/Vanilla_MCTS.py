...
class Node:
    ...
    def ucb1(self):
        ...
        return self.win_num/self.visit_num \
                + Node.CONST_C * np.sqrt(np.log(self.parent_node.visit_num)/self.visit_num)
...
class Vanilla_MCTS:
    ...
    def expansion(self):
        if self.current_node.is_terminal:
            self.winner = self.current_node.terminal_winner
            return 
    ...
    def simulation(self):
        if self.current_node.is_terminal:
            return
        if self.simulator_env.status is not 'going': # draw or win
    ...
    def step(self, action):
        for child in self.root.child_list:
        ...
        self.root = Node(   player = (self.env.next_player+1)%2, 
                            action = None,
